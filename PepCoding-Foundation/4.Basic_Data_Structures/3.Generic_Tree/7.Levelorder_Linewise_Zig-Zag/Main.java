...
public class Main {
  ...
  public static void levelOrderLinewiseZZ(Node root){
    ...
    while(mainS.size() > 0){
        Node fnode = mainS.pop();//remove
        System.out.print(fnode.data+" "); // print

        // add children to helperS normally when the level is even
        if(level%2==0){
        for(Node child : fnode.children){
            helperS.push(child);
        }
        }
        // add children to helperS in reverse when the level is odd
        else{
            for(int i = fnode.children.size()-1; i>=0; i--){
                Node child = fnode.children.get(i); 
                helperS.push(child);
            }
        }
