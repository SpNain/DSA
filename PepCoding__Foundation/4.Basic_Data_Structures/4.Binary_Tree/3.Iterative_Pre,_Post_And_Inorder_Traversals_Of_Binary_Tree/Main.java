...
public class Main {
  ...
  public static void iterativePrePostInTraversal(Node root) {
    ...
    while (st.size() > 0) {
      ...
      if (top.state == 0) { // pre area
        pre += top.node.data + " "; // node ke data ko pre string me dalwao
        if (top.node.left != null) {
          st.push(new Pair(top.node.left, 0));
        }
        top.state++;
      }
      else if (top.state == 1) { // in area
        in += top.node.data + " "; // node ke data ko in string me dalwao
        if (top.node.right != null) {
          st.push(new Pair(top.node.right, 0));
        }
        top.state++;
      }
      else if (top.state == 2) { // post area
        post += top.node.data + " "; // node ke data ko pre string me dalwao
        st.pop();
      }
    }
